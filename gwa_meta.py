...
def main(args):
  ...
  binary = []
  for x in args._in:
    hdr = open(x).readline().strip().split()
    if 'BETA' in hdr: binary.append(False)
    elif 'OR' in hdr: binary.append(True)
    else: binary.append('Warning')
  ...
  if not os.path.isfile(plink_out) or args.force:
    qt = ' + qt' if not any(binary) else ''
    if args.extract != None and os.path.isfile(args.extract):
      qt += f' --extract {args.extract}'
