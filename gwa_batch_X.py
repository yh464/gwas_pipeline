#!/usr/bin/env python3
'''
runs GWAS on traits for the X chromosome
'''
...
def main(args):
    ...
    logout = open(f'{logdir}/gwa_by_trait.log','w')
    ...
    for i in range(c.size):
      mpheno = i+1
      trait = c[i]
      mdl_fname = outdir + trait + f'_{args.maf}_Xmodel'
      out_fname = outdir + trait + f'_{args.maf}_X'
      skip = False
      ...
      submitter.add(f'bash {scripts_path}/gwa_by_trait_X.sh '+
                f'{args.gcta} {args.mb} {args.grm} {args._in}/{f} {mpheno} {args.qcov} {args.dcov} {ft}'+
                f'{mdl_fname} {out_fname} '+
                '--keep /rds/project/rb643-1/rds-rb643-ukbiobank2/Data_Users/yh464/params/ukbkeepfile_X_202402.txt '
                +'--model-only')
    submitter.submit()
...
if __name__ == '__main__':
    ...
    proj.add_output(args.out+'/%pheng/%pheno_%maf_X.fastGWA', __file__)
    try: main(args)
