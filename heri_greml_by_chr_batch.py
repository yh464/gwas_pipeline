#!/usr/bin/env python3
'''
batch runs GREML to assess heritability by chromosome
'''
...
def main(args):
    ...
    submitter = array_submitter.array_submitter(
        name = f'greml_{args.pheno}', n_cpu = 15,
        timeout = 720, lim = 1)
    ...
    for i in range(c.size):
      for chrom in range(1,23):
          mpheno = i+1
          trait = c[i]
          out_fname = outdir + trait + f'_chr{chrom}'
          skip = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--grm', dest = 'grm', help = 'Genetic correlation matrix',
      ...
      default = '/rds/project/rds-Nl99R8pHODQ/UKB/Imaging_genetics/yg330/GRM_chr_merged/GRM.chr%chr')
