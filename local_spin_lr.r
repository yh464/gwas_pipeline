#### Information ####
# Conducts spin permutation tests, using asymmetric Yeo, Mesulam and Economo atlases
    ...
    for (j in 1:length(refname)){
      ...
      if (file.exists(cache_fname)){
        ...
        for (i in 1:10000){
          ...
          tempnull <- merge(z,tempnull,by='Class', all = T)
...
ggsave(paste(prefix,'.spinlr.mes.pdf',sep=''))
ggsave(paste(prefix,'.spinlr.mes.png',sep=''))
