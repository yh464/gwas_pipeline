...
def main(args):
    ...
    for x in args.pheno:
        ...
        for y in os.listdir():
            ...
            df = pd.read_table(y, sep = '\s+', usecols = ['SNP', 'P'], index_col = 'SNP')
            prefix = '.'.join(y.split('.')[:-1]) # remove extension
            df.columns = [prefix]
            all_files.append(df.loc[args.snp,prefix])
        
    all_files = pd.concat(all_files, axis = 1)
    all_files.insert(loc = 0, column = 'min_pval', value = all_files.min(axis = 1))
    print(all_files)
    all_files.to_clipboard(index = True)
